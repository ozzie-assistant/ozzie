...
use worm_memory::{page_frontmatter, MemoryError, PageSearchResult, PageStore, WikiPage};
...
impl MarkdownPageStore {
    ...
    fn create_tables(&self) -> Result<(), MemoryError> {
        ...
        .map_err(|e| MemoryError::Database(format!("create pages tables: {e}")))?;
        Ok(())
    }
    ...
    fn upsert_index(&self, page: &WikiPage, content: &str) -> Result<(), MemoryError> {
        ...
        .map_err(|e| MemoryError::Database(format!("upsert page index: {e}")))?;
        Ok(())
    }
...
impl PageStore for MarkdownPageStore {
    async fn upsert(
        ...
        .map_err(|e| MemoryError::Database(format!("upsert page: {e}")))?;
        drop(conn);
    ...
    async fn get(&self, id: &str) -> Result<(WikiPage, String), MemoryError> {
        ...
        .map_err(|e| match e {
            rusqlite::Error::QueryReturnedNoRows => {
                MemoryError::NotFound(format!("page {id:?}"))
            }
    ...
    async fn get_by_slug(&self, slug: &str) -> Result<(WikiPage, String), MemoryError> {
        ...
        .map_err(|e| match e {
            rusqlite::Error::QueryReturnedNoRows => {
                MemoryError::NotFound(format!("page with slug {slug:?}"))
            }
            other => MemoryError::Database(other.to_string()),
        })
    }
    ...
    async fn delete(&self, id: &str) -> Result<(), MemoryError> {
        ...
        if rows == 0 {
            return Err(MemoryError::NotFound(format!("page {id:?}")));
        }
        Ok(())
    }
