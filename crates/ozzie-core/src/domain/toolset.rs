...
struct ToolSetInner {
    /// Always-on tools.
    core: HashSet<String>,
    /// conversation_id -> activated tool names.
    active: HashMap<String, HashSet<String>>,
    /// conversation_id -> activated-during-turn flag.
...
impl ToolSet {
    ...
    pub fn active_tool_names(&self, conversation_id: &str) -> Vec<String> {
        ...
        if let Some(session_tools) = inner.active.get(conversation_id) {
    ...
    pub fn activate(&self, conversation_id: &str, tool_name: &str) -> bool {
        ...
        inner.turn_flags.insert(conversation_id.to_string(), true);
        true
    }
    ...
    pub fn is_active(&self, conversation_id: &str, tool_name: &str) -> bool {
        let inner = self.inner.read().unwrap();
    ...
    pub fn reset_turn_flag(&self, conversation_id: &str) {
        let mut inner = self.inner.write().unwrap();
        inner.turn_flags.insert(conversation_id.to_string(), false);
    }
    ...
    pub fn activated_during_turn(&self, conversation_id: &str) -> bool {
        let inner = self.inner.read().unwrap();
        inner.turn_flags.get(conversation_id).copied().unwrap_or(false)
    }
    ...
    pub fn cleanup(&self, conversation_id: &str) {
        let mut inner = self.inner.write().unwrap();
        inner.active.remove(conversation_id);
        inner.turn_flags.remove(conversation_id);
    }
}
