...
pub async fn get_profile(State(state): State<AppState>) -> impl IntoResponse {
    match ozzie_core::profile::load(&state.ozzie_path) {
...
pub async fn update_profile(
    ...
    let mut profile = match ozzie_core::profile::load(&state.ozzie_path) {
        Ok(Some(p)) => p,
        Ok(None) => {
    ...
    match ozzie_core::profile::save(&state.ozzie_path, &profile) {
