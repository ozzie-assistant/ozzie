...
mod tests {
    ...
    fn normal_after_grace_decays() {
        let now = Utc::now();
        let old = now - TimeDelta::days(14); // 1 week past grace
        let result = apply_decay(0.8, old, now, ImportanceLevel::Normal);
        assert!(result < 0.8);
        assert!(result > 0.1); // above floor
    }

    #[test]
    fn ephemeral_decays_fast() {
        let now = Utc::now();
        let old = now - TimeDelta::days(8); // 7 days past 1-day grace
    ...
    fn decay_floors() {
        ...
        assert!((result - 0.1).abs() < 0.01); // should be at floor
    }
}
