...
use crate::{MemoryEntry, MemoryError, MemoryType, Store};
...
impl SqliteStore {
    /// Opens (or creates) the SQLite memory database and runs migrations.
...
impl Store for SqliteStore {
    async fn create(&self, entry: &mut MemoryEntry, content: &str) -> Result<(), MemoryError> {
        ...
        if entry.id.is_empty() {
            entry.id = ozzie_utils::names::generate_id("mem", |candidate| {
                conn.query_row(
...
mod tests {
    ...
    async fn fts_search() {
        ...
        store.create(&mut entry, "Rust is a systems programming language").await.unwrap();
