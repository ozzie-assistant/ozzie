...
use crate::{ImportanceLevel, MemoryEntry, MemoryError, MemoryType};
...
/// Parses a simple YAML array: `[a, b, c]` or `["a", "b"]`.
...
pub fn filename_for(entry: &MemoryEntry) -> String {
    ...
    let short_id = if entry.id.len() >= 8 {
        // Take last 4 chars of the ID (after the prefix)
...
fn slugify(text: &str, max_len: usize) -> String {
    ...
    if result.len() > max_len {
        // Find last hyphen before max_len to avoid cutting mid-word
...
mod tests {
    ...
    fn serialize_roundtrip() {
        ...
        assert!(markdown.contains("importance: core"));
        assert!(markdown.contains("# Project Architecture"));
        assert!(markdown.contains("Ozzie follows hexagonal architecture."));
        ...
        assert_eq!(parsed_entry.importance, ImportanceLevel::Core);
    ...
    fn slugify_basic() {
        ...
        assert_eq!(slugify("  spaces  ", 50), "spaces");
    }
